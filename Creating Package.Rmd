...
5. Click on 'creaet in new session'
...
- Out package will only depend on R itself for now, that is no other packages 
...
Got to the README.md file and customize it with a description of the package.

### Adding data to the package

We are going to use the function 'use_data_raw()', this creates a folder called 'data-raw/'' with an R script that will document the creation of the data object that will be stored in our package. 

In this R script called 'DATASET.R' we will document the steps needed to read the raw data, which is in the CSV format, and save it in R format.

1. Add the CSV file to the 'data-raw/' folder.
  - You can copy it from a location in your computer if you have downloaded it previosuly. 
  - You can download it from your computer from [here](http://lunasare.github.io/spring2023-data-science/data/portal-species-taxonomy.csv)
...
This is useful so that the datasets are always available when someone installs an R package.
...
But where are these documention files created and formatted?

There is a special folder called 'man/' in every R package that stores files of type '.Rd', shortcut for 'Rdocument'.

To generate '.Rd' files, first we need to create an .R script in the R/folder called 'data.R'. Copy paste the following lines into the 'data.R' file:
...
## Adding vigenettes to our packages
