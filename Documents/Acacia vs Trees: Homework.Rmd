...
```{r}
library(ggplot2)
FYM <- read.csv(file = "../data-raw/ACACIA_DREPANOLOBIUM_SURVEY.txt", sep = "\t", na.strings = "dead")
ggplot() +
  ...
  geom_line() +
  geom_point(data = FYM, mapping = aes(x = CIRC, y = HEIGHT, color = "red")) +
  scale_x_log10() +
  scale_y_log10() +
  geom_smooth(method = "lm") +
  geom_line()
...
scale_x_log10() +
scale_x_log10() +
geom_smooth(data = trees, mapping = aes(x = CIRC, y= HEIGHT), method = "glm") + 
geom_smooth(data = ACACIA, mapping = aes(x = CIRC, y = HEIGHT), method = "glm")
```